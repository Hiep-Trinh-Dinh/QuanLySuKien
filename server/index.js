...
// GET /events/category/:category_id - Lấy events theo category
// GET /events/category/:category_id/exclude/:event_id - Lấy 3 event khác cùng category
app.get("/events/category/:category_id/exclude/:event_id", async (req, res) => {
  try {
    ...
    if (!category_id || !event_id) {
      return res
        .status(400)
        .json({ message: "Thiếu category_id hoặc event_id!" });
    }
    ...
    console.error(
      "Error in /events/category/:category_id/exclude/:event_id",
      err
    );
    res.status(500).json({ message: "Lỗi server. Vui lòng thử lại." });
  }
...
app.put("/admin/artists/:id", async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [name, bio || null, avatar_url || null, id]
    );
...
app.post("/admin/event-lineup", async (req, res) => {
  try {
    ...
    if (!event_id || !artist_id)
      return res
        .status(400)
        .json({ message: "Thiếu event_id hoặc artist_id." });
    const [result] = await pool.query(
      "INSERT INTO event_lineup (event_id, artist_id, performance_time) VALUES (?, ?, ?)",
      [event_id, artist_id, performance_time || null]
    );
...
app.put("/admin/event-lineup/:id", async (req, res) => {
  try {
    ...
    if (!artist_id)
      return res.status(400).json({ message: "Thiếu artist_id." });
    await pool.query(
      "UPDATE event_lineup SET artist_id = ?, performance_time = ? WHERE id = ?",
      [artist_id, performance_time || null, id]
    );
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        return res
          .status(500)
          .json({ message: "Lỗi khi upload hình ảnh sự kiện." });
      }
    }
      ...
      try {
        if (currentAvatar && currentAvatar.includes("res.cloudinary.com")) {
          ...
          const match = currentAvatar.match(
            /\/upload\/(?:v\d+\/)?(.+)\.(?:jpg|jpeg|png|gif|webp|svg|bmp)/i
          );
        ...
        console.warn(
          "Không thể xóa avatar cũ trên Cloudinary (tiếp tục):",
          err.message || err
        );
      }
...
app.delete("/admin/events/:id", async (req, res) => {
  try {
    ...
    const [tickets] = await pool.query(
      'SELECT COUNT(*) as count FROM tickets WHERE event_id = ? AND status = "sold"',
      [eventId]
    );
    ...
    if (tickets[0].count > 0) {
      return res
        .status(400)
        .json({ message: "Không thể xóa sự kiện đã có vé được bán!" });
    }
...
app.get("/admin/support-tickets", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi server khi lấy danh sách ticket hỗ trợ." });
  }
});
