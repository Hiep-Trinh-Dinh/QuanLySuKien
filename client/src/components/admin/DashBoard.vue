  ...
  <div class="d-flex" style="min-height: 80vh">
    ...
    <main class="flex-grow-1 p-4">
      <div class="d-flex justify-content-end mb-3">
        <button class="btn btn-sm btn-outline-primary" @click="goHome">
          Quay về Home
        </button>
      </div>
...
async function loadOverview() {
  try {
    ...
    stats.value[1].value =
      res.users?.pagination?.total ??
      (Array.isArray(res.users) ? res.users.length : "—");
    stats.value[2].value =
      res.tickets?.pagination?.total ??
      (Array.isArray(res.tickets) ? res.tickets.length : "—");
    stats.value[3].value = "—";
  } catch (err) {
